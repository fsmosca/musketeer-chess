...
#### The drop areas
Drop area of white is at row 0 or squares [A0, B0 ... H0] while black is at row 9 or squares [A9, B9 ... H9]. The pieces in drop areas can only enter the playing area through [gating](#1-gating) moves.
...
The first move in musketeer chess is PS (piece selection). White may select one piece type from the [10 selectable musketeer piece types](#piece-names) and black follows by selecting another piece type. Only 2 piece types are allowed, first is the one selected by white and second the one selected by black.
